/* GLYIV GIP DEMO — reusable per-industry widget: Kalkulator jejak + Agen AI + Offset.
   Usage: <div data-gip-demo="SLUG"></div> ; needs window.CIP_DATA (gip-data.js).
...
(function () {
  ...
  /* ══ HEMAT GERAK — PENJAGANYA HARUS DI SINI, BUKAN DI BERKAS CSS ═══════════
     Gaya widget ini DISUNTIK dari JavaScript, jadi tidak ada berkas .css yang
     bisa dibaca audit statis untuk menemukannya. Akibatnya `cipwPulse` dan
     `cipwSpin` berputar tanpa henti di 13 halaman /lab/industri/* dan di
     /lab/outlet.html — dua animasi per halaman — walau pengguna sudah meminta
     gerak dikurangi. Ditemukan hanya karena disapu di peramban sungguhan
     (`node scripts/sapu-gerak.cjs`).

     Dua pagar dipasang, bukan satu:
       1. `@media (prefers-reduced-motion:reduce)` DI DALAM gaya yang disuntik —
          ikut hidup-mati sendiri saat pengguna mengubah setelannya;
       2. penjaga JS `HEMAT` di bawah — memutuskan SEBELUM elemen beranimasi
          dipasang, dan mendengarkan perubahan setelan (`change`), sehingga
          hitungan angka pun ikut berhenti (rAF tidak terlihat oleh CSS). */
  var HEMAT = (window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)")) || { matches: false, addEventListener: function () {} };
  ...
  var CSS = ''
    ...
    /* Bilah proporsi dulu bergerak lewat `width` — properti TATA LETAK, dan ia
       bergerak setiap kali angka di kalkulator diketik. Kini lebarnya tetap
       penuh dan yang bergerak `scaleX` (lihat `--p` yang disetel recompute). */
    + '.cipw__bar i{display:block;height:100%;width:100%;transform:scaleX(var(--p,0));transform-origin:left center;background:linear-gradient(90deg,var(--ct),#57b97e);border-radius:999px;transition:transform .5s cubic-bezier(.2,.7,.2,1)}'
    ...
    /* ── HEMAT GERAK ────────────────────────────────────────────────────────
       Titik "langsung" dan cincin pemuat tetap TERGAMBAR — hanya geraknya yang
       berhenti — supaya isyarat "ini sedang bekerja" tidak ikut hilang bagi
       pengguna yang meminta gerak dikurangi. `.cipw--diam` adalah pagar kedua
       yang dipasang JS; ia dan media query ini sengaja saling menimpa. */
    + '@media(prefers-reduced-motion:reduce){'
    +   '.cipw *,.cipw *::before,.cipw *::after{animation:none!important;transition-duration:.001ms!important;transition-delay:0s!important}'
    +   '.cipw__insight{opacity:1!important}'
    + '}'
    + '.cipw--diam .cipw__live i,.cipw--diam .cipw__think .sp{animation:none!important}'
    + '.cipw--diam .cipw__bar i,.cipw--diam .cipw__insight,.cipw--diam .cipw__cta{transition:none!important}'
    + '.cipw--diam .cipw__insight{opacity:1}';
  ...
  function build(el, slug) {
    ...
    if (!cfg) { el.innerHTML = '<p style="font-family:monospace;color:#888">GIP demo: data untuk "' + slug + '" belum tersedia.</p>'; return; }
    el.className = "cipw" + (HEMAT.matches ? " cipw--diam" : "");
    /* Setelan bisa berubah SESUDAH halaman dimuat (pengguna mengubahnya di
       sistem). Kelasnya karena itu diikat ke peristiwa, bukan dibaca sekali. */
    var dengar = HEMAT.addEventListener ? "addEventListener" : (HEMAT.addListener ? "addListener" : null);
    if (dengar === "addEventListener") HEMAT.addEventListener("change", function (e) { el.classList.toggle("cipw--diam", e.matches); });
    else if (dengar === "addListener") HEMAT.addListener(function (e) { el.classList.toggle("cipw--diam", e.matches); });
    ...
    function recompute() {
      ...
      ems.forEach(function (kg, i) {
        ...
        b.style.setProperty("--p", (pct / 100).toFixed(4)); b.classList.toggle("hot", i === hotIdx && kg > 0);
      });
    ...
    function tween(node, to, mode) {
      ...
      /* Angka yang berlari juga GERAK, dan ia lahir dari requestAnimationFrame —
         tidak ada satu pun aturan CSS yang bisa menghentikannya. Kalau pengguna
         meminta gerak dikurangi, angkanya langsung mendarat di nilai akhir. */
      if (HEMAT.matches) { node.textContent = f(to); node._v = to; node._raf = 0; return; }
  ...
  function init() {
    inject();
    [].slice.call(document.querySelectorAll("[data-gip-demo]")).forEach(function (el) { build(el, el.getAttribute("data-gip-demo")); });
  }
